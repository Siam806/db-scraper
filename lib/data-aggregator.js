...
function normalizeHeaderKey(header) {
  return normalizeKey(String(header ?? '')) || 'col';
}
...
function uniqueHeaderKeys(headers = []) {
  ...
  return headers.map((header, index) => {
    const base = normalizeHeaderKey(header) || `col_${index + 1}`;
...
function rowArrayToObject(keys, row = []) {
...
function normalizeTables(tables = []) {
  return tables.map((table) => {
    const keys = uniqueHeaderKeys(table.headers);
    return {
      section: table.section,
      headers: table.headers,
      keys,
      rows: table.rows.map((row) => rowArrayToObject(keys, row))
    };
  });
}
...
function tableKeys(table) {
  ...
  return Array.isArray(table.headers) ? table.headers.map(normalizeKey) : [];
}

function isScheduleTable(table) {
  const keys = tableKeys(table);
  return keys.includes('date') || keys.includes('time') || keys.includes('result') || keys.includes('score');
}

function normalizeScheduleRows(tables = []) {
  return tables
...
function normalizeScorelinesRecord(data = {}) {
  const tables = normalizeTables(data.tables || []);
  const standingsTable = findStandingsTable(tables);

  return {
    tables,
    standings: standingsTable ? standingsTable.rows : [],
    schedule: normalizeScheduleRows(tables),
    sections: [...new Set(tables.map((table) => table.section).filter(Boolean))]
  };
}

function normalizeStatisticsRecord(data = {}) {
  const tables = normalizeTables(data.tables || []);
  const sections = buildStatisticsSections(data.tables || []);

  return {
    tables,
    sections,
    groupedByTeam: groupStatisticsByTeam(sections),
...
function normalizeStandings(tables = []) {
  ...
  return table.rows.map((row) => ({
    ...normalizeRow(table.headers, Array.isArray(row) ? row : Object.values(row)),
    section: table.section
  }));
}
...
export function normalizeScorelinesData(data = {}) {
  return normalizeScorelinesRecord(data);
}

export function normalizeStatisticsData(data = {}) {
  return normalizeStatisticsRecord(data);
}
